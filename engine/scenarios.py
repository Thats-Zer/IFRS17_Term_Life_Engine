...
#Bu, senaryoların paralel olarak çalıştırılmasını sağlar.
# Her senaryo, ayrı bir iş parçacığında çalıştırılır ve sonuçlar tamamlandıkça toplanır.
# Bu, özellikle çok sayıda senaryo varsa ve her biri önemli hesaplama gerektiriyorsa, toplam çalışma süresini azaltabilir.

from typing import Optional
#Bu, senaryo sonuçlarının sözlükler halinde döndürüleceğini belirtir.
# Her sözlük, bir senaryonun sonuçlarını içerecektir.

import gc #Bu, senaryo çalıştırmaları sırasında bellek yönetimi için kullanılır.
# Özellikle çok sayıda senaryo çalıştırılırken, bellek kullanımını optimize etmek için gereklidir.

import pandas as pd #Bu, veri manipülasyonu ve analiz için kullanılır.
# Senaryo sonuçlarının toplanması ve analiz edilmesi için DataFrame yapıları sağlar.
...
#Bu, senaryoların çalıştırılması sırasında kullanılacak iskonto eğrisini oluşturmak için 
# kullanılan fonksiyonu içe aktarır.

from engine.projection import (
    
    create_master_data, 
    #Bu, senaryoların çalıştırılması sırasında 
    # kullanılacak ana veri setini oluşturmak için kullanılan fonksiyonu içe aktarır.
   
    create_projection_table,
    #Bu, senaryoların çalıştırılması sırasında
    # kullanılacak projeksiyon tablosunu oluşturmak için kullanılan fonksiyonu içe aktarır.
)

from engine.cashflows import calculate_cashflows
#Bu, senaryoların çalıştırılması sırasında 
# kullanılacak nakit akışlarını hesaplamak için kullanılan fonksiyonu içe aktarır.

from engine.bel import calculate_bel
#Bu, senaryoların çalıştırılması sırasında
# kullanılacak Best Estimate Liability (BEL) hesaplamak için kullanılan fonksiyonu içe aktarır.

from engine.ra import calculate_risk_adjustment
#Bu, senaryoların çalıştırılması sırasında
# kullanılacak Risk Adjustment (RA) hesaplamak için kullanılan fonksiyonu içe aktar
...
def apply_scenario(config: ModelConfig, scenario: dict) -> ModelConfig:
    #Bu fonksiyon, temel yapılandırmayı alır ve senaryo parametreleriyle güncelleyerek
...
def run_single_scenario(
    ...
    try:
        ...
        #Kontrol mekanizması ekleyerek, eğer "Year" sütunu yoksa veya uygun formatta değilse,
        # hata mesajı verir ve senaryonun çalışmasını durdurur.
        ...
        #Kontrol mekanizması ekleyerek, eğer iskonto eğrisi tablosunda "Year" sütunu yoksa veya 
        # uygun formatta değilse, hata mesajı verir ve senaryonun çalışmasını durdurur.
        ...
        if "discount_factor_opening" not in discount_curve.columns:
            ...
            #Bu, iskonto eğrisi tablosunda "discount_factor_opening" sütunu yoksa,
            # "discount_factor" sütununu kopyalayarak "discount_factor_opening" sütununu oluşturur.
        ...
        #Bu, iskonto eğrisi tablosundan "Year", "discount_factor" ve "discount_factor_opening"
        # sütunlarını seçer, ve "Year" sütunundaki tekrar eden değerleri kaldırır.
        ...
        #Bu, projeksiyon tablosunda "coverage_years" sütunu yoksa,
        # "Year" sütununu kopyalayarak "coverage_years" sütununu oluşturur.
        ...
        #Bu, projeksiyon tablosunu kullanarak Best Estimate Liability (BEL) hesaplar
        # sonuçları bel_result DataFrame'ine atar.
        ...
        return {
            ...
            # Eğer yapılandırmada "n_policies" parametresi varsa onu kullanır,
            # yoksa master_data'daki kayıt sayısını kullanır.
            "Total_BEL": total_bel,
    ...
    # hata mesajı yazdırılır ve None döndürülür.
    #None döndürülmesi, bu senaryonun sonuçlarının geçersiz olduğunu ve
    # sonraki işlemlerde dikkate alınmaması gerektiğini belirtir.
...
def run_scenarios(
    ...
    #Bu fonksiyon, verilen senaryoları paralel olarak çalıştırır ve sonuçları
    # bir DataFrame olarak döndürür.
    max_workers = int(getattr(base_config, "scenario_max_workers", 1) or 1)
    #Bu, senaryoların paralel olarak çalıştırılacağı maksimum iş parçacığı sayısını belirler.
    # Eğer yapılandırmada "scenario_max_workers" parametresi varsa onu kullanır,
