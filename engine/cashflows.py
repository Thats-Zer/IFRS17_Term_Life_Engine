...
def allocate_expenses(projection: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    """
    ...
    if config.admin_cost_per_policy < 0:
        raise ValueError(
            f"admin_cost_per_policy >= 0 olmalı, {config.admin_cost_per_policy} verildi"
        )
    if config.collection_cost_rate < 0 or config.collection_cost_rate > 1:
        raise ValueError(
            f"collection_cost_rate [0, 1] olmalı, {config.collection_cost_rate} verildi"
        )
    ...
    projection["acquisition_cost"] = np.where(
        projection["Year"] == 1, float(config.acquisition_cost) * expense_mult, 0
    ).astype(np.float32)
    ...
    projection["total_expense_base"] = (
        projection["acquisition_cost"] + projection["admin_cost"] + projection["maintenance_cost"]
    ).astype(np.float32)
...
def calculate_surrender_charges(projection: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    """
    ...
    if config.surrender_charge_rate < 0 or config.surrender_charge_rate > 1:
        raise ValueError(
            f"surrender_charge_rate [0, 1] olmalı, {config.surrender_charge_rate} verildi"
        )
    ...
    projection["surrender_charge"] = (
        surrender_base * config.surrender_charge_rate * surrender_multiplier
    ).astype(np.float32)
...
def calculate_annual_premiums(projection: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    """
    ...
    projection["premium_annuity_pv"] = (
        in_force * projection["survival_ratio"] * projection["discount_factor_opening"]
    ).astype(np.float32)
    ...
    premium_calc = projection.groupby("policy_id", sort=False, as_index=False).agg(
        {
            "unit_claim_pv": "sum",
            "unit_expense_pv": "sum",
    ...
    total_loading = config.profit_margin + config.contingency_loading + config.collection_cost_rate
    ...
    if total_loading >= 1:
        ...
        premium_calc["gross_annual_premium"] = (
            premium_calc["net_annual_premium"] / (1 - total_loading)
        ).astype(np.float32)
    ...
    projection = projection.merge(
        premium_calc[["policy_id", "net_annual_premium", "gross_annual_premium"]],
        on="policy_id",
        how="left",
    )
    ...
    projection["net_annual_premium"] = (
        projection["net_annual_premium"].fillna(0.0).astype(np.float32)
    )
    projection["gross_annual_premium"] = (
        projection["gross_annual_premium"].fillna(0.0).astype(np.float32)
    )
...
def calculate_cashflows(projection: pd.DataFrame, config: ModelConfig) -> pd.DataFrame:
    """
    ...
    projection["Gross_Premium_Inflow"] = (
        in_force * projection["survival_ratio"] * projection["gross_annual_premium"]
    ).astype(np.float32)

    # Net premium inflow (cost basis için)
    projection["Net_Premium_Inflow"] = (
        in_force * projection["survival_ratio"] * projection["net_annual_premium"]
    ).astype(np.float32)

    # Collection cost: premium'un yüzdesi (scenario expense multiplier dahil)
    projection["collection_cost"] = (
        projection["Gross_Premium_Inflow"] * float(config.collection_cost_rate) * expense_mult
    ).astype(np.float32)
    ...
    projection["adjusted_qx"] = (
        (projection["qx"] * (1 - lapse_mortality_correlation * projection["lapse_rate"]))
        .clip(0, 1)
        .astype(np.float32)
    )
    ...
    projection["Reinsurance_Ceding"] = (
        projection["Gross_Premium_Inflow"] * float(config.reinsurance_cost_rate)
    ).astype(np.float32)

    projection["Reinsurance_Recovery"] = (
        projection["Death_Benefits"] * float(config.reinsurance_cost_rate)
    ).astype(np.float32)
    ...
    projection["Counterparty_Default_Cost"] = (
        projection["Reinsurance_Recovery"] * config.counterparty_pd * config.counterparty_lgd
    ).astype(np.float32)

    # Net death benefit (reinsurance recovery sonrası)
    projection["Net_Death_Benefit"] = (
        projection["Death_Benefits"] - projection["Reinsurance_Recovery"]
    ).astype(np.float32)
    ...
    projection["Total_Inflows"] = (
        projection["Gross_Premium_Inflow"] + projection["Reinsurance_Recovery"]
    ).astype(float_dtype)

    projection["Net_Cash_Flow"] = (
        projection["Total_Inflows"] - projection["Total_Outflows"]
    ).astype(float_dtype)
    ...
    for col in opening_cols:
        if col in projection.columns:
            projection[f"PV_{col}"] = (
                projection[col] * projection["discount_factor_opening"]
            ).astype(float_dtype)

    for col in closing_cols:
        if col in projection.columns:
            projection[f"PV_{col}"] = (projection[col] * projection["discount_factor"]).astype(
                float_dtype
            )

    projection["PV_Total_Inflows"] = (
        projection["PV_Gross_Premium_Inflow"] + projection["PV_Reinsurance_Recovery"]
    ).astype(float_dtype)
    ...
    projection["PV_Net_Cash_Flow"] = (
        projection["PV_Total_Inflows"] - projection["PV_Total_Outflows"]
    ).astype(float_dtype)
