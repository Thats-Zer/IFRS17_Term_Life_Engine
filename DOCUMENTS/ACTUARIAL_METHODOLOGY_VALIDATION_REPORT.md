...
Diagnostics are optional and controlled by `enable_bel_diagnostics`. When enabled, the audit report includes a structured BEL diagnostic summary under `bel_diagnostics`.
...
Diagnostic metadata attached to the BEL summary:

```text
run_type: base | scenario
scenario_name: null or scenario key
diagnostic_source: calculate_bel
bel_convention: liability_positive
formula: BEL = PV(outflows) - PV(inflows)
```

To avoid global-state risks when scenarios run in the same process, base-run diagnostics
are captured immediately after the base BEL calculation and passed explicitly to the
output/audit layer. Scenario runs disable diagnostics by default.
...
Expected results may vary as tests evolve; run the suite to validate the current state.
...
- diagnostics are optional and may be disabled for large production-scale runs
...
The model is suitable for CV and interview discussion, especially for actuarial internship, junior actuarial analyst, insurance risk, or valuation-oriented roles.# IFRS 17 Term Life Engine
